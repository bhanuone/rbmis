class Admin::SpecialitiesController < ApplicationController
  ...
  def index
    @specialities = Speciality.includes(:treatment_category).all
  end
  ...
  private
    ...
    def set_speciality
      @speciality = Speciality.includes(:treatment_category).find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def speciality_params
      params.require(:speciality).permit(:name, :description, :treatment_category_id)
    end
end
